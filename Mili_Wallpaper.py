...
def open_log_json():
    if os.path.exists(f"{os.path.split(__file__)[0]}/Log/log.log"):
        with open(f"{os.path.split(__file__)[0]}/Log/log.log", "r", encoding="utf-8") as rfp:
            Video = rfp.read()
    else:
        """创建"""
        with open(f"{os.path.split(__file__)[0]}/Log/log.log", "w+", encoding="utf-8") as wfp:
            wfp.write("None")
            Video = None
    return Video

"""写入缓存数据log.log文件（用于下次用户打开程序不需要再次选择视频文件）"""
def write_log_json(FilePath):
    if os.path.exists(os.path.split(__file__)[0]+"/Log/log.log"):
        with open(os.path.split(__file__)[0]+"/Log/log.log", "w+", encoding="utf-8") as wfp:
            wfp.write(FilePath)
        return True
    else:
        return False
...
def ReadVideoList():
    if os.path.exists(os.path.split(__file__)[0]+"/Log/VideoList.json"):
        with open(os.path.split(__file__)[0]+"/Log/VideoList.json", "r", encoding="utf-8") as rfp:
            return json.loads(rfp.read())['videolist']
    else:
        with open(os.path.split(__file__)[0]+"/Log/VideoList.json", "w+", encoding="utf-8") as wfp:
            wfp.write(json.dumps({"videolist":[]}))
            return []

def WriteVideoList(JsonData):
    with open(os.path.split(__file__)[0] + "/Log/VideoList.json", "w+", encoding="utf-8") as wfp:
        wfp.write(json.dumps(JsonData))
    return True


"""本地视频播放"""
def PathPlay(Play_Video, width, height, audio, volume, ISWHIEL=" -loop 0 "):
    if audio == True:
        os.popen(f"cd {os.path.split(__file__)[0]}/ && ffplay -x {width} -y {height} -i {Play_Video} -volume {volume}{ISWHIEL}-threads 16 -noborder -window_title FFPlay：{Play_Video}")
    else:
        os.popen(f"cd {os.path.split(__file__)[0]}/ && ffplay -x {width} -y {height} -i {Play_Video} -an{ISWHIEL}-threads 16 -noborder -window_title FFPlay：{Play_Video}")
    Log.record("正常运行", f"ffmpeg Play -->> {Play_Video} Width:{width} height:{height} audio:{audio} volume:{volume}")

"""本地视频播放线程"""
def ThreadPlay(Play_Video, width, height, audio, volume, kill, sleeptime, ISWHIEL=" -loop 0 "):
    kill()
    Th = Thread(target=PathPlay, args=(Play_Video, width, height, audio, volume, ISWHIEL))
    Th.start()
    Th.join()
...
def GetListBoxSet(ListBox, VideoList, label, BgPIL):
    def BoxSet(Box):
        while True:
            try:
                x = list(Box.curselection())[0]
                # print(VideoList[x])
                bg = BgPIL(VideoList[x])
                label.config(image=bg)
                label.update()
            except IndexError:
                bg = BgPIL(VideoList[0])
                try:
                    label.config(image=bg)
                    label.update()
                except _tkinter.TclError:
                ...
                break
            time.sleep(0.1)
    Thread(target=BoxSet, args=(ListBox, )).start()

def PlayVideoList(VideoList, width, height, audio, volume, kill, sleeptime):
    line = 0
    while True:
        _id_ = int(open(f"{os.path.split(__file__)[0]}/log/UIDLOG.log", "r", encoding="utf-8").read())
        try:
            IFTrue = int(json.loads(open(f"{os.path.split(__file__)[0]}/Log/VideoList.json", "r", encoding="utf-8").read())['IFWHILE'])
        except:
            time.sleep(1)
            IFTrue = int(json.loads(open(f"{os.path.split(__file__)[0]}/Log/VideoList.json", "r", encoding="utf-8").read())['IFWHILE'])

        try:
            Video = VideoList[line]
        except IndexError:
            line = 0
            Video = VideoList[line]
        if IFTrue == 0:
            win32gui.PostMessage(_id_, win32con.WM_CLOSE, 0, 0)
            break
        elif Window_settings.Isffmpeg(Window_settings._id_):
            continue
        else:
            kill()
            PathPlay(Video, width, height, audio, volume, " -autoexit ")
            Window_settings.main(f"FFPlay：{Video}", sleeptime)
            UIDLOG(Window_settings._id_)
            Log.record("正常运行", "Settings：True")
            line += 1

def FFpaly_PlayVideoList(VideoList, width, height, audio, volume, kill, sleeptime):
    Play = Thread(target=PlayVideoList, args=(VideoList, width, height, audio, volume, kill, sleeptime))
    Play.start()
...
class Mili_Wallpaper:
    ...
    def __init__(self):
        ...
        self.Play_Video_List = ReadVideoList()
    ...
    def main(self):
        ...
        xMenu.add_command(label="自动更换壁纸(ctrl+p+s)", command=self.PlayVideoList)
        ...
        Label(self.tk, text=f"Version/版本:{__Version__}", bg="black", fg="white").place(x=65, y=1)
    ...
    def destroy_all_gui(self, event=False):
        with open("./Log/VideoList.json", "w+", encoding="utf-8") as wfp:
            wfp.write(json.dumps({
                "videolist": self.Play_Video_List,
                "IFWHILE": 0
            }))
        sys.exit(0)
        self.tk.destory()

    def Select_FileList(self, event=False):
        file = tkinter.Tk()
        file.withdraw()
        FileList = tkinter.filedialog.askopenfilenames(title="选择多个壁纸(按住ctrl键多选)")
        if FileList:
            FilesJson = {"videolist":FileList}
            WriteVideoList(FilesJson)
            self.Play_Video_List = FileList
            return True
        else:
            return False

    def PlayVideoList(self, event=False):
        PATH = os.path.split(__file__)[0]
        if self.Play_Video_List:
            _playvideolist = Toplevel()
            _playvideolist.resizable(False, False)
            _playvideolist.geometry("960x540")
            _playvideolist.iconbitmap(f"{PATH}/image/mili_wallpaper.ico")

            def ReadVideoFPSImage(Video):
                global frame_image
                cap = cv2.VideoCapture(Video)
                IS, frame = cap.read()
                cv2image = cv2.cvtColor(frame, cv2.COLOR_BGR2RGBA)
                arryimage = Image.fromarray(cv2image).resize((960, 540))
                frame_image = ImageTk.PhotoImage(arryimage)
                return frame_image

            BgImage = Label(_playvideolist, width=960, height=540)
            BgImage.pack()

            VideoList = Listbox(_playvideolist, width=50)
            VideoList.place(x=0, y=0)
            for video in self.Play_Video_List:
                VideoList.insert("end", os.path.split(video)[-1])

            GetListBoxSet(VideoList, self.Play_Video_List, BgImage, ReadVideoFPSImage)

            """音量"""
            Label(_playvideolist, text="音量：", font=("", 10), bg="black", fg="white", height=3).place(x=10, y=480)
            volume = tkinter.Scale(_playvideolist, from_=1, to=100, orient=HORIZONTAL, tickinterval=10, length=300, borderwidth=1,font=("", 8), width=6)
            volume.place(x=55, y=480)
            volume.set(100)
            ...
            def PlayFile():
                ...
                FFpaly_PlayVideoList(
                    self.Play_Video_List,
                    _playvideolist.winfo_screenwidth(),
                    _playvideolist.winfo_screenheight(),
                    True,
                    volume.get(),
                    self.kill,
                    self.PlayerJson['delay_local']
                )

            Button(_playvideolist, text="确定", bg="black", fg="white", command=PlayFile).place(x=380, y=490)
            def KillWHILE():
                with open("./Log/VideoList.json", "w+", encoding="utf-8") as wfp:
                    wfp.write(json.dumps({
                        "videolist":self.Play_Video_List,
                ...
                self.kill()
            Button(_playvideolist, text="关闭壁纸", bg="black", fg="white", command=KillWHILE).place(x=420, y=490)

            _playvideolist.mainloop()
        else:
            if self.Select_FileList():
                self.PlayVideoList()
            else:
                pass

    def SeeLogin(self, event=False):
        PATH = os.path.split(__file__)[0]
        SeeLog = Toplevel()
    ...
    def Select_file(self, event=False):
        ...
        if os.path.isfile(FilePath):
            ...
            with open(os.path.split(__file__)[0]+"/Log/LogVideo/PlayFile"+os.path.splitext(_filename)[-1], "wb") as wfp:
                wfp.write(ReadFile)
            write_log_json(os.path.split(__file__)[0]+"/Log/LogVideo/PlayFile"+os.path.splitext(_filename)[-1])
            self.Play_Video = os.path.split(__file__)[0]+"/Log/LogVideo/PlayFile"+os.path.splitext(_filename)[-1]
            return os.path.split(__file__)[0]+"/Log/LogVideo/PlayFile"+os.path.splitext(_filename)[-1]
        else:
            return False
            ...
            if os.path.exists(open_log_json()):
                if os.path.exists(f"{os.path.split(__file__)[0]}/Log/VideoConfig.log"):
                    if Window_settings.Isffmpeg(open(f"{os.path.split(__file__)[0]}/Log/UIDLOG.log", "r", encoding="utf-8").read()) == False:
                        config = [self.tk.winfo_screenwidth(), self.tk.winfo_screenheight(), True, 100]
                        try:
                            with open(f"{os.path.split(__file__)[0]}/Log/VideoConfig.log", "r", encoding="utf-8") as fp:
                                VideoConfig = fp.read().split(';')
                                Log.record("正常运行", config)
                                if VideoConfig[0] == 'True':
                                    config[2] = True
                                else:
                                    config[2] = False
                                config[-1]=int(VideoConfig[-1])
                        except:
    ...
    def Play(self, event=False):
        ...
        if os.path.exists(self.Play_Video):
            ...
            def play():
                ...
                with open(f"{os.path.split(__file__)[0]}/Log/VideoConfig.log", "w+", encoding="utf-8") as fp:
                    fp.write(f"{self.audio};{volume.get()}")
                if os.path.exists(self.Play_Video):
                    if os.path.exists(f"{os.path.split(__file__)[0]}/Log/VideoConfig.log"):
                        ThreadPlay(
                            self.Play_Video,
    ...
    def kill(self, event=False):
        ...
        try:
            ...
            win32gui.SendMessage(win32gui.FindWindow("Progman", "Program Manager"), 0x052C, 0xD, 0)
            win32gui.SendMessage(win32gui.FindWindow("Progman", "Program Manager"), 0x052C, 0xD, 1)
        except:
            pass
    ...
    def Version_update(self, event=False):
        ...
        if req.status_code == 200:
            for key, value in req.json().items():
                if __Version__ >= key:
                    tkinter.messagebox.showinfo("程序已是最新版本！", f"程序已是最新版本！{__Version__}")
                else:
                    tkinter.messagebox.showinfo("当前程序不是最新版本", f"程序最新下载地址：{value}")
                    os.system(f"start {os.path.split(value)[0]}/Mili_Wallpaper_Version")
        else:
    ...
    def Clear_cache(self, event=False):
        self.kill()
        open(f"{os.path.split(__file__)[0]}/Log/log.log", "w+", encoding="utf-8").write("None")
        open(f"{os.path.split(__file__)[0]}/Log/VideoConfig.log", "w+", encoding="utf-8").write("True;100")
        open(f"{os.path.split(__file__)[0]}/Log/SelfStartingPlayConfig.wll", "w+", encoding="utf-8").write("关闭自启动壁纸;True")
            ...
            if len(files):
                for file in files:
                    try:
                        ...
                        Log.record(f"删除文件 -->> {paths}/{file}")
                    except:
                        continue

        for paths, dirs, files in os.walk(f"{os.path.split(__file__)[0]}/Log/LogVideo"):
            if len(files):
                for file in files:
                    try:
                        os.remove(paths+"/"+file)
        ...
        with open(f"{os.path.split(__file__)[0]}/Log/VideoList.json", "w+", encoding="utf-8") as wfp:
            wfp.write(json.dumps({
                "videolist":[],
                "IFWHILE":1
            }))
