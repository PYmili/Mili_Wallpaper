...
"""本地视频播放"""
def PathPlay(Play_Video, width, height, audio, IS_FS):
    if IS_FS:
        if audio == True:
            os.popen(f"cd {os.path.split(__file__)[0]}/ && ffplay -fs -i {Play_Video} -loop 0 -threads 16 -noborder -window_title FFPlay：{Play_Video}")
        else:
        ...
        if audio == True:
            os.popen(f"cd {os.path.split(__file__)[0]}/ && ffplay -x {width} -y {height} -i {Play_Video} -loop 0 -threads 16 -noborder -window_title FFPlay：{Play_Video}")
        else:
...
"""本地视频播放线程"""
def ThreadPlay(Play_Video, width, height, audio, kill, sleeptime, IS_FS=True):
    Th = Thread(target=PathPlay, args=(Play_Video, width, height, audio, IS_FS))
    Th.start()
    Th.join()
...
class Mili_Wallpaper:
                ...
                if os.path.exists(f"{os.path.split(__file__)[0]}/Log/VideoConfig.log"):
                    ...
                    try:
                        ...
                        with open(f"{os.path.split(__file__)[0]}/Log/VideoConfig.log", "w+", encoding="utf-8") as fp:
                            fp.write(f"1920;1080;True")
                        config = [1920, 1080, True]
    ...
    def Play(self, event=False):
        ...
        if os.path.exists(self.Play_Video):
            ...
            def play():
                ...
                with open(f"{os.path.split(__file__)[0]}/Log/VideoConfig.log", "w+", encoding="utf-8") as fp:
                    fp.write(f"{self.width};{self.height};{self.audio}")
                if os.path.exists(self.Play_Video):
                    if os.path.exists(f"{os.path.split(__file__)[0]}/Log/VideoConfig.log"):
    ...
    def Clear_cache(self, event=False):
        ...
        with open(f"{os.path.split(__file__)[0]}/Log/log.log", "w+", encoding="utf-8") as f:
            f.write(str(None))
        open(f"{os.path.split(__file__)[0]}/Log/VideoConfig.log", "w+", encoding="utf-8").write("1920;1080;True")
        open(f"{os.path.split(__file__)[0]}/Log/SelfStartingPlayConfig.wll", "w+", encoding="utf-8").write("关闭自启动壁纸;True")
