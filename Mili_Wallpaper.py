...
"""本地视频播放"""
def PathPlay(Play_Video, width, height, audio, volume, IS_FS, ISWHIEL=" -loop 0 "):
    if IS_FS:
        if audio == True:
            os.popen(f"cd {os.path.split(__file__)[0]}/ && ffplay -fs -i {Play_Video} -volume {volume}{ISWHIEL}-threads 16 -noborder -window_title FFPlay：{Play_Video}")
        else:
            os.popen(f"cd {os.path.split(__file__)[0]}/ && ffplay -fs -i {Play_Video} -an{ISWHIEL}-threads 16 -noborder -window_title FFPlay：{Play_Video}")
...
"""本地视频播放线程"""
def ThreadPlay(Play_Video, width, height, audio, volume, kill, sleeptime, IS_FS=True, ISWHIEL=" -loop 0 "):
    kill()
    Th = Thread(target=PathPlay, args=(Play_Video, width, height, audio, volume, IS_FS, ISWHIEL))
    Th.start()
    Th.join()
...
def PlayVideoList(VideoList, width, height, audio, volume, kill, sleeptime):
    line = 0
    while int(json.loads(open(f"{os.path.split(__file__)[0]}/Log/VideoList.json", "r", encoding="utf-8").read())['IFWHILE']):
        try:
        ...
        if Window_settings.Isffmpeg(Window_settings._id_):
            continue
        else:
            kill()
            if int(cap.get(3)) < width:
                PathPlay(Video, width, height, audio, volume, False, " -autoexit ")
            else:
                PathPlay(Video, width, height, audio, volume, True, " -autoexit ")
